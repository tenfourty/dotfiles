...
keyboards_UserCustom_dct = {
    ...
    # --- xps13 built-in keyboard: deliberately classified as Apple ----------
    # This is NOT a misidentification fix -- it is a deliberate role swap.
    #
    # Physical layout is:  Ctrl . Fn . Win . Alt . Space
    #
    # As 'Windows', Toshy makes the ALT key (next to space) Cmd, which matches
    # the Mac *positionally* but not the operator's muscle memory.
    #
    # An Apple keyboard reports its Cmd key as LEFT_META -- the same evdev code
    # the Win key sends. So classifying this keyboard as 'Apple' yields:
    #     Win  (LEFT_META) -> Cmd      <-- Win+C / Win+V / Win+Left ...
    #     Alt  (LEFT_ALT)  -> Option
    #     Ctrl (LEFT_CTRL) -> Super in GUI apps, stays Ctrl in terminals
    # which is exactly the requested behaviour, using a supported override
    # rather than editing the generated modmaps (which are lost on upgrade).
...
if DESKTOP_ENV == 'kde':
    # ---- macOS Mission Control / App Expose equivalents ---------------------
    #
    # Ctrl+Up   -> Mission Control  (KDE "Overview")
    # Ctrl+Down -> App Expose       (KDE "Present Windows (Window class)")
    #
    # Placed in the user_apps slice deliberately: xwaykeyz resolves keymaps in
    # DEFINITION ORDER and returns on the first one containing the combo. This
    # slice (~line 4034) registers before the built-in "GenGUI/GenTerms
    # overrides: KDE" blocks (~line 5970), so these take precedence -- and it
    # survives Toshy upgrades, unlike editing the generated blocks.
    #
    # Input side: physical Ctrl becomes Super in GUI apps but stays Left Ctrl
    # in terminals, hence the two keymaps.
    #
    # Output side: combos were verified against what KDE has *live* via
    #   gdbus ... org.kde.kglobalaccel.Component.allShortcutInfos
    # not against what kglobalshortcutsrc appears to say:
    #   Overview    -> Meta+W   active [268435543]
    #   ExposeClass -> Ctrl+F7  active [83886134]
    #                  (Meta+F7 = 285212726 is a *default* but is NOT bound)
    keymap("User: KDE Mission Control - GUI", {
        C("Super-Up"):              C("Super-w"),           # Mission Control
        C("Super-Down"):            C("C-F7"),              # App Expose
    }, when = lambda ctx:
        cnfg.screen_has_focus and
        ctx_ovl_macos_globals and
        not ctx_app_is_remote
    )
    keymap("User: KDE Mission Control - Terminals", {
        C("LC-Up"):                 C("Super-w"),           # Mission Control
        C("LC-Down"):               C("C-F7"),              # App Expose
    }, when = lambda ctx:
        cnfg.screen_has_focus and
